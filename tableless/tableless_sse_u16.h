...
class ViterbiDecoder_Tableless_SSE_u16
{
public:
    using poly_t = get_register_type<constraint_length>;
    using Config = Tableless_Config<int16_t, poly_t>;
private:
...
public:
    ...
    static sum_error_t update(Base& base, const int16_t* symbols, const size_t N, const Config& config) {
...
private:
    static void bfly(Base& base, const int16_t* symbols, decision_bits_t* decision, uint16_t* old_metrics, uint16_t* new_metrics, const Config& config) {
        ...
        constexpr size_t SIZE_IN_BYTES = sizeof(int16_t)*NUMSTATES;
        ...
        for (size_t curr_state = 0u; curr_state < v_stride_branch_table; curr_state++) {
            const size_t curr_state_offset = curr_state*8;
            // Total errors across R symbols
            __m128i total_error = _mm_set1_epi16(0);
            for (size_t i = 0u; i < Base::R; i++) {
                const __m128i branch_value = mm128_get_branch_value_s16(curr_state_offset, config.poly[i], config.soft_decision_low, config.soft_decision_high);
                __m128i error = _mm_subs_epi16(branch_value, v_symbols[i]);
