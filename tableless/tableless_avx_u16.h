...
class ViterbiDecoder_Tableless_AVX_u16
{
public:
...
private:
    static void bfly(Base& base, const int16_t* symbols, decision_bits_t* decision, uint16_t* old_metric, uint16_t* new_metric, const Config& config) {
        ...
        for (size_t curr_state = 0u; curr_state < v_stride_branch_table; curr_state++) {
            ...
            for (size_t i = 0u; i < Base::R; i++) {
                const __m256i branch_value = mm256_get_branch_value_s16(curr_state_offset, config.poly[i], config.soft_decision_low, config.soft_decision_high);
                __m256i error = _mm256_subs_epi16(branch_value, v_symbols[i]);
