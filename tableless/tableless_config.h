...
template <size_t K> struct GetRegisterType<K, typename std::enable_if<K <= 8>::type> { using type = uint8_t; };
template <size_t K> struct GetRegisterType<K, typename std::enable_if<K <= 16>::type> { using type = uint16_t; };
template <size_t K> struct GetRegisterType<K, typename std::enable_if<K <= 32>::type> { using type = uint32_t; };
